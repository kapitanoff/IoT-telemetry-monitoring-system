...
IoT Telemetry Monitoring System - система мониторинга телеметрии IoT-датчиков
через MQTT. Проект принимает температуру и напряжение питания от датчиков или
программного эмулятора, сохраняет данные, показывает состояние объектов на
web-панели и считает метрики качества доставки сообщений.
...
В качестве демонстрационного сценария используется мониторинг температуры
куриц на птицеферме: каждая карточка соответствует датчику, закрепленному за
отдельной птицей. Архитектура при этом не привязана к этой предметной области и
может использоваться для других MQTT-датчиков.
...
```powershell
git clone https://github.com/kapitanoff/IoT-telemetry-monitoring-system.git
cd IoT-telemetry-monitoring-system
```
...
только базовый `docker-compose.yml`, без локального Mosquitto из
`docker-compose.override.yml`.
...
пороговые значения температуры. Если скрипт спросит, запускать ли систему сразу,
для внешнего брокера лучше ответить `n`, а затем запустить только базовый compose:
